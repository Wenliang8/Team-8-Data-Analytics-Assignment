...
non_diabetic = df_new[df_new['Outcome'] == 0]
diabetic = df_new[df_new['Outcome'] == 1]
...
for col in ['Pregnancies', 'SkinThickness', 'Glucose','BloodPressure','BMI','DiabetesPedigreeFunction','Age']:
  stat, p = mannwhitneyu(non_diabetic[col], diabetic[col], alternative='two-sided')
...
non_diabetic = df_new[df_imputed['Outcome'] == 0]
...
for col in ['Pregnancies', 'SkinThickness', 'Glucose','BloodPressure','BMI','DiabetesPedigreeFunction','Age']:
    ...
    stat, p = ttest_ind(non_diabetic[col], diabetic[col], nan_policy='omit')
    sig = 'Significant' if p < 0.05 else 'Not Significant'
...
print(f'\n Dataset: {df_new.shape[0]} records, {df_new.shape[1]} features')
print(f' Missing values: None')
print(f'\n Class Imbalance:')
print(f'   Non-diabetic: {(df["Outcome"]==0).sum()} ({(df["Outcome"]==0).mean()*100:.1f}%)')
print(f'   Diabetic: {(df["Outcome"]==1).sum()} ({(df_new["Outcome"]==1).mean()*100:.1f}%)')
print(f'\n Correlations with Diabetes Diagnosis Outcome:')

# Get the correlation series
correlation_series = df_new[num_cols].corr()['Outcome'].drop('Outcome')
...
for col in num_cols:
    ...
    if col == 'Outcome':
        continue
    t, p = stats.ttest_ind(non_diabetic[col], diabetic[col])
